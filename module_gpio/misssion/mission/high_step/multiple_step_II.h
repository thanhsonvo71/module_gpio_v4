...
int multiple_step_II::action(int action){
    ...
    if(action==Active_){
        if(my_multiple_step_II.size()!=0){
            ...
            if(my_step!=NULL) value_return=my_step->action(Active_);
...
void multiple_step_II::delete_free(){
    if(my_multiple_step_II.size()==0 & my_step!=NULL){
        my_step->delete_free();
        delete my_step;
    }
    for(int i=0;i<my_multiple_step_II.size();i++){
        if(my_multiple_step_II[i]!=NULL){
